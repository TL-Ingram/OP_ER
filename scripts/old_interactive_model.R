qlik_interactive_model <- function(q,additions_factor, removals_factor, user, specialty_name, division_name ){
  library(parallel)
  library(dplyr)
  library(ggplot2)
  library(tidyr)
  ...
  #save.image("C:/R/Scripts/MASTER/int_data.RData")
  #load("int_data.RData")
  
  # Get date of current WL
  ...
  wl$planned_admission_date = as.Date(
    ifelse(
      ...
      as.Date(wl$decision_to_admit_date_dt + 2 * 365 * 24 * 60 * 60),
  ...
  if(type == "current_rates"){
    ...
    capacity <-
      rep(
        ...
        h
      )* as.numeric(removals_factor)/100
    ...
    demand <-
      rep(
        ...
        h
      ) * as.numeric(additions_factor)/100
    ...
    if(dim(wl)[1] != 0){
      ...
      n.runs <- 50 # number of runs of the simulation
      ...
      res.sum$division  = division_name
      res.sum$creator  = user
      res.sum$model    = "interactive_rates"
      
      # Write to table
      print(paste0("Removing old results from the table..."))
      q_rm <-
        paste0("DELETE from elective_recovery_model_outputs where creator = '" ,user,"' AND model = 'interactive_rates' AND specialty   = '", specialty_name,"'")
      ...
      q_rm <-
        paste0("DELETE from elective_recovery_model_clearance_times where creator = '",user,"' AND model = 'interactive_rates' AND specialty   = '", specialty_name,"'")
