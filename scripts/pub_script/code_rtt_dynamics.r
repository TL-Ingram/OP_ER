...
### INPUTS ###
rep(1,4+2)
clock.starts<-rep(12,200+365)  # vector containing num clock starts for each day in considered period
clock.stops<-rep(10,200+365)   # vector containing max num clock stops (capacity) for each day in considered period
...
initial.waiting.list<-1000     # initial number of incomplete referrals (i.e. with no clock stop)
...
simfn<-function(SEED) {
  ...
  res<-data.frame(id=1:initial.waiting.list,time_arr=0
                  ,priority=sample(1:length(priority.prob),
                                   initial.waiting.list,replace=TRUE,priority.prob),stringsAsFactors=FALSE)
  ID<-initial.waiting.list
  outp<-data.frame(day=0,
                   act=NA,maxwt=NA,meanwt=NA,medianwt=NA)
  ...
  for (d in 1:nrow(rates)) {
    ...
    outp<-rbind(outp,data.frame(day=d,act=d_srv,maxwt=maxwt,meanwt=meanwt,medianwt=medianwt))
  }
